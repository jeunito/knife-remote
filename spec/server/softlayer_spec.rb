...
describe "softlayer server" do
  context "device status" do
    it "is on" do
      ...
      expect(Chef::Config).to receive(:[]).with(:knife).and_return({:softlayer_api_key => "api_key", :softlayer_user => "user" })
      ...
      expect(api).to receive(:service_named).with("Accounts").and_return(accounts_service)

      expect(accounts_service).to receive(:object_filter).and_return(accounts_filter)
      expect(accounts_filter).to receive(:object_mask).with("mask[id,fullyQualifiedDomainName]").and_return(accounts_mask)
